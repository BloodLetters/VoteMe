...
use pumpkin::{net::java::config, plugin::Context};
...
async fn on_load(&mut self, server: Arc<Context>) -> Result<(), String> {
    ...
    log::info!("Config loaded - Host: {}, Port: {}, RSA Bits: {}", 
        config.host, config.port, config.rsa_bits);
    ...
    let privkey = if Path::new("plugins/VoteMe/rsa/private.key").exists() {
        ...
        log::info!("private.key not found, generating new RSA keypair with {} bits...", config.rsa_bits);
                ...
                match listener.accept().await {
                    Ok((mut socket, addr)) => {
                        ...
                        tokio::spawn(async move {
                            if (config.debug) { log::debug!("Accepted vote connection from {}", addr) } 
                            ...
                            if (config.debug) { log::debug!("Vote connection closed: {}", addr) }
                        });
                    }
                    Err(e) => {
